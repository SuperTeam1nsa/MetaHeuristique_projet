...
public class RecuitSolver implements Solver {
	
	private int max_iter;
	private int temperature_ini;
	private int iter_temperature_maj;
	//en seconde
	public RecuitSolver(int max_iter, int temperature_ini, int  iter_temperature_maj) {
		this.max_iter=max_iter;
		this.temperature_ini=temperature_ini;
		this.iter_temperature_maj=iter_temperature_maj;
	}
...
//anonymous fonction doesn't allow modification of local variable inside
    ...
    public Result solve(Instance instance, long deadline) {
        ResourceOrder current;
        ResourceOrder voisin;
        ResourceOrder best;
        int bestMakespan;
        int k =0;
    	...
    	do{
    		...
    		swaps=new ArrayList<Swap>();
    		 //on extrait chaque block du chemin critique et pour chacun on enregistre tous les swap possibles
    		 ...
    		 //on choisit un voisin au hasard et on calcule son makespan
    		 int nombreAleatoire =  (int)(Math.random() * (swaps.size()));
    		 voisin=current.copy();
    		 swaps.get(nombreAleatoire).applyOn(voisin);
			 int neighborsMakespan =voisin.toSchedule().makespan();
			 //s'il améliore il devient notre nouveau meilleur
			 if(neighborsMakespan < bestMakespan) {
    			 bestMakespan=neighborsMakespan;
        		 best=voisin;
        		 current=voisin.copy();
			 }else {
				 //s'il n'améliore pas, il a quand même une probabilité d'être conservé (règle dite de Métropolis)
    		 double deltaF= bestMakespan-neighborsMakespan;
    		 double proba = Math.exp(-deltaF/T);
    		 //si chanceux on accepte 
	    		 if(Math.random() < proba){
	    		 current=voisin.copy();
	    		 }
			 }
    		 if(k% this.iter_temperature_maj==0)
    			 T--;
    		 
    	 } while((deadline - System.currentTimeMillis() > 1) && k <max_iter && T >= 0);
    	//Timeout ou max iter
    	if(k == max_iter)
